...
class Create:
    def run(self, options):
        ...
        if os.path.exists(basedir):
            print("Refusing to touch pre-existing directory %s" % basedir, file=stderr)
            return 1
        ...
        if not options["quiet"]:
            ...
            print("TubID %s, listening on port %s" % (tub.getTubID(),
                                                      options["port"]),
                  file=stdout)
            print("Now launch the daemon with 'flappserver start %s'" % basedir,
                  file=stdout)
...
class List:
    def run(self, options):
        ...
        for s in list_services(basedir):
            ...
            print(" %s %s" % (s.service_type, " ".join(s.service_args)),
                  file=stdout)
            if s.comment:
...
class Start:
    def run(self, options):
        ...
        for fn in os.listdir(basedir):
            ...
            print("%s does not look like a node directory (no .tac file)" % basedir,
                  file=stderr)
            return 1
...
class Stop:
    def run(self, options):
        ...
        if not try_to_kill(pid, signal.SIGTERM):
            ...
            print("process %d wasn't running, removing twistd.pid to cleanup" % pid,
                  file=stderr)
            return 2
        ...
        print("Process %d didn't respond to SIGTERM, sending SIGKILL." % pid,
              file=stderr)
